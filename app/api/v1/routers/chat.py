...
from app.api.deps import get_ngent_client, require_permission
...
async def create_session(
    ...
    _: Principal = Depends(require_permission("chat:ask")),
...
async def list_sessions(
    ...
    data = await ngent.request("GET", "/v1/threads")
...
async def list_messages(
    ...
    data = await ngent.request("GET", f"/v1/threads/{session_id}/history?includeEvents=1")
...
async def rename_session(
    ...
    data = await ngent.request("PATCH", f"/v1/threads/{session_id}", json={"title": body.title})
...
async def cancel_chat_task(
    ...
    data = await ngent.request("POST", f"/v1/turns/{task_id}/cancel")
    return {"taskId": task_id, "status": data.get("status", "cancel_requested")}


@router.get("/tasks/{task_id}/position")
async def chat_task_position(task_id: str, _: Principal = Depends(require_permission("chat:ask"))) -> dict:
...
async def chat_task_events(
    ...
    async def events():
        async for line in ngent.stream("GET", f"/v1/turns/{task_id}/events"):
            yield f"{line}\n"
